...
window.addEventListener('DOMContentLoaded', () => {
	...
	const CONFIGURATION = {
		...
		"mapOptions": {"center":{"lat":51.5169057,"lng":-0.06462459999999999},"fullscreenControl":true,"mapTypeControl":false,"streetViewControl":false,"zoom":14,"zoomControl":true,"maxZoom":20,"mapId":""},
		"mapsApiKey": "AIzaSyDD8R7YovtasMKrrOIp8D29AoFiJH8fKSE"
	};
	
	const commutesEl = {
		map: document.querySelector('.map-view'),
		initialStatePanel: document.querySelector(
	...
	const destinationPanelEl = {
		...
		list: commutesEl.destinationPanel.querySelector(
			'.destination-list',
		),
		...
		getActiveDestination: () =>
			commutesEl.destinationPanel.querySelector(
				'.destination.active',
			),
	};
	...
	window.initMap = async function () {
		 ...
		 */
		 function Commutes(configuration) {
				...
				for (const destination of configuration.initialDestinations) {
					...
					placesService.getDetails(
						...
						function (place) {
							...
							const travelModeEnum = destination.travelModeEnum ||
							...
							getDirections(destinationConfig).then(
								(response) => {
									if (!response) return;
									...
									if (
										callbackCounter ===
											configuration.initialDestinations
												.length
									) {
										...
										for (
											...
											bounds.union(
												destinations[i].bounds,
											);
										}
										const lastIndex = destinations.length -
											1;
										handleRouteClick(
											destinations[lastIndex],
											lastIndex,
										);
						...
						() => {
							console.error(
								'Failed to retrieve places info due to ' + e,
							);
						},
					);
				}
			...
			function initCommutesPanel() {
				const addCommutesButtonEls = document.querySelectorAll(
					'.add-button',
				);
				addCommutesButtonEls.forEach((addButton) => {
					addButton.addEventListener('click', () => {
						...
						const travelModeEnum =
							configuration.defaultTravelModeEnum ||
							TravelMode.DRIVING;
						const travelModeId = travelModeEnum.toLowerCase() +
							'-mode';
						document.forms['destination-form'][travelModeId]
							.checked = true;
					});
				});
			...
			function initCommutesModal() {
				...
				const destinationFormReset = function () {
					destinationModalEl.destinationInput.classList.remove(
						'error',
					);
				...
				destinationModalEl.addButton.addEventListener('click', () => {
					const isValidInput = validateDestinationInput(
						destinationToAdd,
					);
				...
				destinationModalEl.editButton.addEventListener('click', () => {
					...
					getDirections(destination)
						.then((response) => {
							...
							const newEditButton = destinationPanelEl.list
								.children
						...
						.catch((e) =>
							console.error(
								'Editing directions failed due to ' + e,
							)
						);
					hideModal();
				});

				destinationModalEl.cancelButton.addEventListener(
					'click',
					() => {
						destinationFormReset();
						hideModal();
				...
				destinationModalEl.deleteButton.addEventListener(
					'click',
					() => {
						...
						if (destinations.length) {
							...
							elToFocus = destinationPanelEl
								.getActiveDestination();
						} else {
							elToFocus = commutesEl.initialStatePanel
				...
				commutesEl.modal.addEventListener('keydown', (e) => {
					switch (e.key) {
						case 'Enter':
							...
							if (
								destinationModalEl.addButton.style.display !==
									'none'
							) {
								destinationModalEl.addButton.click();
							} else if (
								destinationModalEl.editButton.style.display !==
									'none'
							) {
				...
				for (const el of lastInteractiveElements) {
					el.addEventListener(
						'keydown',
						handleLastInteractiveElementTab,
					);
				}
			...
			function validateDestinationInput(destinationToAdd) {
				...
				if (!destinationToAdd) {
					...
					errorMessage = 'Cannot add more than ' +
						MAX_NUM_DESTINATIONS + ' destinations';
				} else if (
					destinations &&
					destinations.find(
						(destination) =>
			...
			function buildDestinationCardTemplate(
				...
				switch (destinationOperation) {
					case DestinationOperation.ADD:
						...
						editButtonEl = destinationContainerEl.querySelector(
							'.edit-button',
						);
					...
					case DestinationOperation.EDIT:
						...
						activeDestinationContainerEl.innerHTML =
							generateDestinationTemplate(destination);
						activeDestinationContainerEl.addEventListener(
							'click',
							() => {
								handleRouteClick(destination, destinationIdx);
							},
						);
						editButtonEl = activeDestinationContainerEl
							.querySelector('.edit-button');
						break;
				...
				editButtonEl.addEventListener('click', () => {
					...
					document.forms['destination-form'][travelModeId].checked =
						true;
			...
			function assignMapObjectListeners(destination, destinationIdx) {
				...
				google.maps.event.addListener(
					...
					() => {
						handleRouteClick(destination, destinationIdx);
						destinationPanelEl.list.querySelectorAll(
							'.destination',
			...
			function handleRouteClick(destination, destinationIdx) {
				if (activeDestinationIndex !== undefined) {
					...
					destinations[activeDestinationIndex].polylines.innerStroke
						.setOptions(
							{
								strokeColor: STROKE_COLORS.inactive.innerStroke,
								zIndex: 2,
					...
					destinations[activeDestinationIndex].polylines.outerStroke
						.setOptions(
							{
								strokeColor: STROKE_COLORS.inactive.outerStroke,
								zIndex: 1,
					...
					const activeDestinationEl = destinationPanelEl
						.getActiveDestination();
			...
			function convertDurationValueAsString(durationValue) {
				...
				const minutes = Math.floor(
					durationValue % HOUR_IN_SECONDS / 60,
				);
		...
		function handlePanelScroll() {
			...
			if (scrollWidth > width) {
				if (position === 0) {
					destinationPanelEl.scrollLeftButton.classList.remove(
						'visible',
					);
				} else {
					destinationPanelEl.scrollLeftButton.classList.add(
						'visible',
					);
				}
				...
				if (Math.ceil(position + width) >= scrollWidth) {
					...
					destinationPanelEl.scrollRightButton.classList.add(
						'visible',
					);
				}
			}
