...
window.addEventListener('DOMContentLoaded', () => {
	...
	window.initAutocomplete = async function () {
		...
		sender_address_details = document.querySelector(
			'[name=sender_address_details]',
		);
		...
		recipient_address_details = document.querySelector(
			'[name=recipient_address_details]',
		);
		recipient_postcode = document.querySelector(
		...
		try {
			for (const component of place.address_components) {
				...
				switch (componentType) {
					case 'street_number': {
						sender_address1 =
							`${component.long_name} ${sender_address1}`;
						break;
					}
					...
					case 'postal_code': {
						sender_postcode1 =
							`${component.long_name}${sender_postcode1}`;
						break;
					}
					case 'postal_code_suffix': {
						sender_postcode1 =
							`${sender_postcode1}-${component.long_name}`;
			...
			for (const component of place.address_components) {
				...
				switch (componentType) {
					case 'street_number': {
						...
						recipient_address1 =
							`${component.long_name} ${recipient_address1}`;
						break;
					}
					...
					case 'postal_code': {
						recipient_postcode1 =
							`${component.long_name}${recipient_postcode1}`;
						break;
					}
					case 'postal_code_suffix': {
						recipient_postcode1 =
							`${recipient_postcode1}-${component.long_name}`;
