window.addEventListener('DOMContentLoaded', () => {
	...
	script.src =
		'https://maps.googleapis.com/maps/api/js?key=AIzaSyBMN0tTYpnYsYcy62DPJoLB_bqZqHnNVDU&&libraries=geometry&callback=initMap&v=weekly';
	script.async = true;
	...
	window.initMap = async function () {
		...
		Promise.all([
			...
			let pos2 = value[1].results[0].geometry.location;
			console.log(pos1);
			...
			const contentString2 =
				...
				`<p style="font-size: 14px; margin-bottom: 5px;"><b>Address:</b> ${endFullAdd}` +
				'<hr>';
	...
	function display(status) {
		...
		map_error.innerHTML =
			`The map could not be displayed status code: <strong>${status}</strong> <br>
