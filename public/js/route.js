...
function Commutes(configuration) {
	...
	function initCommutesPanel() {
		...
		addCommutesButtonEls.forEach((addButton) => {
			addButton.addEventListener('click', () => {
				...
				showModal();
				const travelModeEnum = configuration.defaultTravelModeEnum ||
					TravelMode.DRIVING;
	...
	function initCommutesModal() {
		...
		destinationModalEl.deleteButton.addEventListener('click', () => {
			...
			if (destinations.length) {
				...
				elToFocus = commutesEl.initialStatePanel.querySelector(
					'.add-button',
				);
			}
			hideModal(elToFocus);
	...
	function validateDestinationInput(destinationToAdd) {
		...
		if (!destinationToAdd) {
			...
			errorMessage = 'Cannot add more than ' + MAX_NUM_DESTINATIONS +
				' destinations';
		} else if (
			destinations &&
			destinations.find(
	...
	function buildDestinationCardTemplate(
		...
		switch (destinationOperation) {
			...
			case DestinationOperation.EDIT:
				...
				editButtonEl = activeDestinationContainerEl.querySelector(
					'.edit-button',
				);
				break;
