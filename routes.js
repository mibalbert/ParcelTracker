...
import { getParcels,  getParcelsCustomer, getParcelsAccepted } from './modules/retrieve.js'
...
router.post('/home-courier-transit', async context => {
	...
	const result = await setParcelStatus(context, data)
	console.log(result)
	const parcels = await getParcelsAccepted()
	data = { alert: result, parcels }    
...
router.post('/home-courier-receiver-details/:uuid', async context => {
	...
	const alert = await setParcelStatusDelivered(uuid, data)
